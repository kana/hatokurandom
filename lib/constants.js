...
function chooseRandomCards (cardCount, options) {
...
export function ogpMetaFromPid (pid) {
  ...
  const title = titleFromPid(pid)
  const description = xcards.map(xcard => xcard.name).join('、')
  const version = 1
  const image = `https://hatokurandom.whileimautomaton.net/ogp/${pid}?v=${version}`
  const url = `https://hatokurandom.whileimautomaton.net/${pid}`
