...
export function cidsFromPid (pid) {
  const delayedCids = PID_TO_DELAYED_CIDS_TABLE[pid]
  if (delayedCids !== undefined) {
    return force(delayedCids)
  }

  const parsed = parseSpecialPid(pid)
  if (parsed.random) {
    return chooseRandomCids(parsed.count)
  }

  return undefined
}

function chooseRandomCids (count) {
  ...
  return chosenCards.map(card => card.cid)
}
