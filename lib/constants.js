...
export function xcardsFromPid (pid) {
  ...
  if (delayedCids !== undefined) {
    return force(delayedCids).map(card => ({
      ...card,
      dropped: false
    }))
  }
...
export function parseSpecialPid (pid) {
  ...
  // TODO: Support supply:editor
  // TODO: Support supply:XXXXXX
...
function encodeBase64 (sixBitByteBuffer) {
  return sixBitByteBuffer.map((b) => {
    ...
    if (c === undefined) {
      throw new RangeError(`Invalid value to encodeBase64: ${b}`)
    }
    return c
  }).join('')
}
...
function decodeBase64 (base64EncodedString) {
  return base64EncodedString.split('').map((c) => {
    ...
    if (b === undefined) {
      throw new RangeError(`Invalid value to decodeBase64: ${c}`)
    }
    return b
  })
}
...
export function xcardsFromRsid (rsid) {
  ...
  if (bs.length !== 0) {
    throw new RangeError(`Invalid rsid with trailing data: ${JSON.stringify(bs)}`)
  }
