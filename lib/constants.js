...
export const EXPANSIONS = [ // {{{
  { eid: EID_BASIC, name: '基本セット', symbol: '基本', optionKey: 'include_basic' },
  { eid: EID_FAREAST, name: '極東辺境領', symbol: '極東', optionKey: 'include_fareast' },
  { eid: EID_NORTHERN, name: '北限の魔女', symbol: '北限', optionKey: 'include_northern' },
  { eid: EID_FAIRY, name: 'フェアリーガーデン', symbol: 'ＦＧ', optionKey: 'include_fairy' },
  { eid: EID_SIX, name: '六都市同盟', symbol: '六都', optionKey: 'include_six' },
  { eid: EID_STAR, name: '星天前路', symbol: '星天', optionKey: 'include_star' },
...
const COMMON_CARDS = ALL_CARDS.filter(card => card.rarity === 'C')
...
export function titleFromPid (pid) {
  return PID_TO_TITLE_TABLE[pid] || 'ランダムサプライ'
}
...
export function xcardsFromPid (pid) {
  ...
  if (parsed.random) {
    return chooseRandomCards(parsed.count).map(card => ({
      ...card,
      dropped: false
    }))
  }
...
function chooseRandomCards (count) {
  // TODO: Option - Always include cards from specific expansions
  // TODO: Option - Exclude cards from specific expansions
  // TODO: Option - ...
  const restCards = COMMON_CARDS.slice()
  const chosenCards = []
  for (let i = 1; i <= count && restCards.length >= 1; i++) {
