...
//      Note that most of page ids are named as "{TYPE}:{VALUE}" for historical reasons.
//      It might be better to change this naming like "{TYPE}/{VALUE}" for various reasons.
//      (Using ":" in path is uncommon, and handling such URLs is a bit difficult in Nuxt.)
...
export const LOCATION_ORIGIN = 'https://hatokurandom.whileimautomaton.net'
...
const CARD_NAME_TO_CID_TABLE = new Map(ALL_CARDS.map(card => [card.name, card.cid]))

export function cidFromCardName (cardName) {
  return CARD_NAME_TO_CID_TABLE.get(cardName)
}

async function cardTextFromCid (cid) {
  const cardTextData = await import('~/lib/cardText.json')
  const card = cardFromCid(cid)
  return cardTextData[card.name]
}

export async function cardTextComponentFromCid (cid) {
  const cardText = await cardTextFromCid(cid)
  return makeCardTextComponent(cardText || ['（データが見つかりませんでした。）'])
}
...
export function titleFromPid (pid) {
  ...
  const card = parseCardDetailPid(pid)
  if (card) {
    return card.name
  }
  ...
  if (parsed.special && parsed.xcards) {
    return parsed.origin === 'edited' ? '手動作成' : 'ランダム'
  }

  return '???'
}

export function titleTagValueFromPid (pid) {
  if (pid === 'home') {
    return 'ハトクランダム'
  }
  ...
  return parentTitle && parentTitle.match(/^(推奨サプライ|世界選手権)/)
    ? `${titleFromPid(pid)} | ${parentTitle} | ハトクランダム`
    : `${titleFromPid(pid)} | ハトクランダム`
}

const PID_TO_DESCRIPTION_TABLE = { // {{{
  home: 'ハートオブクラウンを楽しく遊ぶ為のランダマイザです。'
} // }}}

export function descriptionFromPid (pid) {
  const description = PID_TO_DESCRIPTION_TABLE[pid]
  if (description) {
    return description
  }
...
export function isPreferencesTabPid (pid) {
  return pid.startsWith('preferences')
}
...
export function chooseRandomCards (cardCount, options) {
  ...
  const mustCids = new Set(mustXcards.map(xcard => xcard.cid))
  const availableCards = COMMON_CARDS.filter(card =>
    !mustCids.has(card.cid) &&
    card.cid !== changedXcard.cid &&
    ...
    (options.excludeCardsWithConditionalActivatedAbility ? !isCardWithConditionalActivatedAbility(card) : true) &&
    !options.excludeBannedCardsByUser.includes(card.cid)
  )
  function weightFromCid (cid) {
    // Used count | Weight
    // -----------|-------
    //         0  |     25
    //         1  |     16
    ...
    //         4  |      1
    //         5+ |      1
    const n = Math.max(1, 5 - (recentlyUsedCountFromCid[cid] || 0))
    return n * n
  }
  const tryCount = 1000
  let _try = 0
  while (true) {
    // Algorithm A-Res, described in https://en.wikipedia.org/wiki/Reservoir_sampling
    //
    // Note that A-Res with fastpriorityqueue takes 0.116 ms, while uniform
    // random sampling takes 0.117 ms.  These times are measured by choosing 10
    // cards 100k times with the default preferences.
    const n = Math.max(0, cardCount - mustXcards.length)
    const cks = availableCards.map(card => ({
      card,
      key: Math.random() ** (1 / weightFromCid(card.cid)) // Should use [0, 1] RNG.
    }))
    const heap = new Heap(function (a, b) { return a.key < b.key })
    for (const ck of cks.slice(0, n)) {
      heap.add(ck)
    }
    for (const ck of cks.slice(n)) {
      if (heap.peek().key < ck.key) {
        heap.poll()
        heap.add(ck)
      }
    }
    const chosenCards = heap.kSmallest(n).map(ck => ck.card)
    for (const xcard of mustXcards) {
      chosenCards.push(xcard)
    }
...
function isCardWithConditionalActivatedAbility (card) {
  return card.name === '灯台' ||
    card.name === '義賊' ||
    card.name === '先行投資' ||
    card.name === '家庭教師' ||
...
export function parseSpecialSupplyPid (pid) {
  ...
  try {
    const { xcards, origin } = xcardsAndMetaFromRsid(maybeRsid)
    return {
      editable: false,
      origin,
      special: true,
...
export function parseCardDetailPid (pid) {
  const matched = pid.match(/^cards\/([1-9][0-9]*)$/)
  if (!matched) {
    return undefined
  }

  return cardFromCid(parseInt(matched[1], 10))
}

export function isCardDetailPid (pid) {
  return pid.startsWith('cards/')
}

export function isCardListPid (pid) {
  return PID_TO_DELAYED_CIDS_TABLE[pid] !== undefined ||
    parseSpecialSupplyPid(pid).special
}
...
export function rsidFromXcards (xcards, isManuallyEdited = false) {
  // See also xcardsAndMetaFromRsid.
  const bs = []

  const version = 0x01
  const origin = (isManuallyEdited ? 1 : 0) << 5
  const meta = version | origin
  bs.push(meta)
...
export function xcardsAndMetaFromRsid (rsid) {
  // Example:
  //
  //   rsid = "BAXBGBMBOBPBTB0B4CCCJ"
  //        = meta ("B") + xcards ("AX" + "BG" + "BM" + ... + "B4" + "CC" + "CJ")
  //
  //   | meta      | xcards[0]          | xcards[1]          | ...
  //   |-----------|--------------------|--------------------|-----
  //   |         B |         A |      X |         B |      G | ...
  //   |-----------|-----------|--------|-----------|--------|-----
  //   | 0   00001 | 0   00000 | 010111 | 0   00001 | 000110 | ...
  //   |-----------|--------------------|--------------------|-----
  //   | 0 | 00001 | 0 | 00000   010111 | 0 | 00001   000110 | ...
  //   |---|-------|---|----------------|---|----------------|-----
  //   | 0 |  0x01 | 0 |           0x17 | 0 |           0x46 | ...
  //   |-----------|---|----------------|---|----------------|-----
  //   | ^ |version| ^ |            cid | ^ |            cid | ...
  //   | | |       | | |      (補給部隊)| | |(ギルドマスター)|
  //     |           |                    |
  ...
  //
  // The first character in an rsid represents contains the metadata of that
  // rsid.  Its MSB represents how xcards were chosen.  The other 5 bits is
  // a version of rsid/xcards conversion.
  //
  ...
  const meta = bs.shift()
  const version = meta & ~(1 << 5)
  const origin = meta & (1 << 5) ? 'edited' : 'random'
  if (version !== 0x01) {
...
export function pidFromPath (path) {
  // Note that path might include ?query=parameter.
  return (path.slice(1) || 'home').replace(/\?.*/, '')
}
...
export function excerptFromPid (pid) {
  const parentPid = parentPidFromPid(pid)
  if (parentPid) {
    const match = titleFromPid(parentPid).match(/^(?:推奨サプライ（(.*)）|(世界選手権))$/)
    if (match) {
...
export function transition (to, from) {
  const lastTouchTime = getLastTouchTime()
  const currentTime = new Date().valueOf()
  const shouldEnableTransition = (
    // Is navigated via custom component or custom gesture?
    to.params.transition ||
    // Or, seems NOT to be navigated via Safari's gesture?
...
function isForwardTransitionByRoutes (to, from) {
  // On page load, from is undefined.
  if (!from) {
    return true
  }

  return isForwardTransitionByPids(pidFromPath(to.path), pidFromPath(from.path))
}

export function isForwardTransitionByPids (to, from) {
  if (to === 'home') {
    return false
  }

  if (!isPreferencesTabPid(to) && isPreferencesTabPid(from)) {
    return false
  }

  // TODO: Use store.getters['history/backPath'] to avoid these ad hoc rules.
  if (to === 'supplies:log' && from !== 'home') {
    return false
  }
  if (to === 'about' && from !== 'home') {
    return false
  }
  if (isCardDetailPid(from)) {
    return false
  }

  const childPids = childPidsFromPid(to)
  if (childPids) {
    return !childPids.includes(from)
  }
...
export function permalinkFromPid (pid) {
  return `${LOCATION_ORIGIN}${pathFromPid(pid)}`
}
...
function metaFromPid (pid) {
  if (!pid.includes('supply:')) {
    return metaFromPidForFallback(pid)
  }
  ...
  return metaFromPidForSupply(pid)
}

function metaFromPidForFallback (pid) {
  return {
    url: permalinkFromPid(pid),
    title: titleFromPid(pid),
    description: descriptionFromPid(pid),
    image: `${LOCATION_ORIGIN}/apple-touch-icon.png`,
    card: 'summary'
  }
}

function metaFromPidForSupply (pid) {
  const xcards = sortXcards(xcardsFromPid(pid)).filter(xcard => !xcard.dropped)
  const parentPid = parentPidFromPid(pid)
  const parentTitle = parentPid ? titleFromPid(parentPid) : null

  const title = parentTitle && parentTitle.match(/^(推奨サプライ|世界選手権)/)
  ...
  return {
    url: permalinkFromPid(pid),
    title,
    description: xcards.map(xcard => xcard.name).join('、'),
    image: ogpImageUrlFromPid(pid),
    card: 'summary_large_image'
  }
}

export function ogpMetaFromPid (pid) {
  const { url, title, description, image, card } = metaFromPid(pid)
  return [
    { hid: 'og:url', property: 'og:url', content: url },
    { hid: 'og:title', property: 'og:title', content: title },
    { hid: 'og:description', property: 'og:description', content: description },
    { hid: 'og:image', property: 'og:image', content: image },
    { hid: 'twitter:card', property: 'twitter:card', content: card },
...
function makeCardTextComponent (cardText) {
  return {
    components: {
      CardName: () => import('~/components/CardText/CardName'),
      Coin: () => import('~/components/CardText/Coin'),
      Mode: () => import('~/components/CardText/Mode'),
      Note: () => import('~/components/CardText/Note'),
      SuccessionPoint: () => import('~/components/CardText/SuccessionPoint'),
      Trigger: () => import('~/components/CardText/Trigger'),
      Unit: () => import('~/components/CardText/Unit')
      // XXX: Define components for decoration.
    },
    render (h) {
      return h('div', cardText.map(t => markUpCardTextSingleAbility(h, t)))
    }
  }
}

function markUpCardTextSingleAbility (h, text) {
  return text.split('\n').map(t => h('unit', markUp(h, t)))
}

function markUp (h, text) {
  ...
  if (m[2] && m[3]) {
    xs.push(h(m[2], markUp(h, m[3])))
  }
  if (m[4]) {
    xs.push(markUp(h, m[4]))
  }
  return xs
}
