...
export function xcardsFromPid (pid, options = {}, mustXcards = []) {
  ...
  const parsed = parseSpecialPid(pid)
  if (parsed.random) {
    return chooseRandomCards(parsed.count, options, mustXcards).map(card => ({
      ...card,
      dropped: false
    }))
  }
...
export function chooseRandomCards (cardCount, options, mustXcards) {
