...
export function chooseRandomCards (cardCount, options) {
  const { mustXcards = [], changedXcard = {} } = options
  ...
  function weightFromCid (cid) {
    return 1 // TODO: Calculate from recently used supplies.
  }
  const tryCount = 1000
  let _try = 0
  while (true) {
    ...
    //
    // A-Res takes 0.230 ms, while uniform random sampling only takes 0.117 ms.
    // (These are measured by choosing 10 cards 100k times with the default preferences.)
    ...
    const heap = new Heap(
      ...
      // So heap.pop() returns the minimum value.
    ...
    const chosenCards = heap.toArray().map(ck => ck.card)
