...
export function chooseRandomCards (cardCount, options) {
  ...
  while (true) {
    ...
    const chosenCards = availableCards.slice(0, n)
    let totalWeight = sum(chosenCards.map(card => weightFromCid(card.cid)))

    for (let i = n; i < availableCards.length; i++) {
      // TODO: Resulting selection seems to be completely wrong...
      const currentCardWeight = weightFromCid(availableCards[i].cid)
      totalWeight += currentCardWeight
      const p = currentCardWeight / totalWeight
      const j = Math.random() // [0, 1), although this should be [0, 1].
      if (j >= p) {
        const k = Math.floor(Math.random() * n)
        chosenCards[k] = availableCards[i]
      }
    }
