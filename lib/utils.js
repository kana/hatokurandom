...
export async function cardTextHtmlsFromCid (cid) {
  const cardText = await cardTextFromCid(cid)
  return markUpCardText(cardText)
}
...
function markUpCardText (cardText) {
  return cardText.map(markUpCardTextSingleAbility)
}

function markUpCardTextSingleAbility (text) {
  let t = text

  t = t.replace(/<coin>(.+?)<\/coin>/g, '<span class="coin">$1</span>')
